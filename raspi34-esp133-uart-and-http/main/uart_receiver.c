...
// ===== UART設定 =====
#define UART_PORT       UART_NUM_0     // USB-UARTブリッジ経由（デフォルトGPIO 43/44）
...
static void receiver_task(void *arg)
{
    ...
    while (1) {
        ...
        if (hdr.payload_len != (uint32_t)(1200u * 1600u / 2u)) {
            ESP_LOGW(TAG, "bad len: %" PRIu32, hdr.payload_len);
        ...
        if (err == ESP_OK) {
            ...
            uart_write_str("OK draw\n");
        } else {
            ESP_LOGE(TAG, "Draw failed: %d", err);
            uart_write_str("ERR draw\n");
        }
    }
}
...
void uart_receiver_start(void)
{
    ...
    // UART0はデフォルトでGPIO 43/44（USB-UARTブリッジ）を使用するためピン指定不要

    ESP_LOGI(TAG, "UART%d: baud=%d", UART_PORT, UART_BAUD);

    xTaskCreatePinnedToCore(receiver_task, "uart_receiver", 8192, NULL, 5, NULL, 1);
